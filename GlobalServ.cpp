...
void GlobalServer::Init(unsigned int threads, int tasks)
{
    ...
    g_GlobalServ->net_io = new NetIO;
    g_GlobalServ->net_io->Init("127.0.0.1", 16070, g_GlobalServ->m_WorkQue, g_GlobalServ->m_RecvQue);

    for (int i = 0; i < g_GlobalServ->m_ProcessNum; i++)
    {
        g_GlobalServ->m_WorkProcessList[i].init(g_GlobalServ->m_WorkQue, g_GlobalServ->m_RecvQue, g_GlobalServ->net_io->m_poLongConn, g_GlobalServ->net_io->m_uLockServer);
    }
...
bool GlobalServer::Start()
{
    ...
    pthread_create(&thIOId, NULL, NetIO::RoutineNetIO, g_GlobalServ->net_io);
